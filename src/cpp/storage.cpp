...
std::shared_ptr<Storage> Storage::from_iterable(py::sequence values,
...
std::shared_ptr<Storage> Storage::clone() const {
  ...
  if (device() == Device::Cpu)
    ...
    NT_CUDA_CHECK(cudaMemcpy(new_storage->data(), data(), nbytes,
...
std::shared_ptr<Storage> Storage::to(Device target_device) const {
  ...
  size_t nbytes = static_cast<size_t>(size()) * itemsize();
  cudaMemcpyKind kind = (device() == Device::Cpu) ? cudaMemcpyHostToDevice
...
std::shared_ptr<Storage> Storage::cast(Dtype target) const {
  ...
  DispatchAll::run(dtype(), [&]<class Src>() {
    DispatchAll::run(target, [&]<class Dst>() {
...
py::buffer_info Storage::buffer_info() const {
  return py::buffer_info(const_cast<void *>(data()), itemsize(),
                         dtype_to_format(dtype()), size());
}
...
void bind_storage_(py::module_ &m) {
  ...
  py::class_<Storage, std::shared_ptr<Storage>>(m, "Storage",
                                                py::buffer_protocol())
      .def_static("allocate", &Storage::allocate, py::arg("n"),
                  py::arg("dtype"), py::arg("device"))
      .def_static("from_iterable", &Storage::from_iterable, py::arg("values"),
                  py::arg("dtype"), py::arg("device"))
