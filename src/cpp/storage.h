...
public:
  ...
  static std::shared_ptr<Storage> allocate(py::ssize_t n, Dtype dtype,
                                           Device device);
  static std::shared_ptr<Storage> from_iterable(py::sequence values,
...
public:
  ...
  Scalar(py::object obj) {
    if (py::isinstance<py::bool_>(obj))
      ...
      data_ = obj.cast<int64_t>();
    else if (py::isinstance<py::float_>(obj))
      data_ = obj.cast<double>();
    else
...
inline void _require_same_device(const std::shared_ptr<Storage> s1,
                                 const std::shared_ptr<Storage> s2,
                                 const char *fname) {
  if (s1->device() != s2->device())
    throw std::runtime_error(std::string(fname) +
                             ": Expected two storages on the same device.");
}

inline void _require_same_dtype(const std::shared_ptr<Storage> s1,
                                const std::shared_ptr<Storage> s2,
                                const char *fname) {
  if (s1->dtype() != s2->dtype())
    throw std::runtime_error(std::string(fname) +
                             ": Expected two storages on the same dtype.");
}
