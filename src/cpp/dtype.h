...
inline std::string dtype_to_format(Dtype dtype) {
  switch (dtype) {
  case Dtype::Bool:
    return std::string(FORMAT_BOOL);
  case Dtype::Int32:
    return std::string(FORMAT_INT32);
  case Dtype::Int64:
    return std::string(FORMAT_INT64);
  case Dtype::Float32:
    return std::string(FORMAT_FLOAT32);
  case Dtype::Float64:
    return std::string(FORMAT_FLOAT64);
  default:
    throw std::invalid_argument("Unknown dtype.");
  }
}
